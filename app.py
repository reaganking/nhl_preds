...
def write_html(preds: List[Dict[str, Any]], path: str, report_date: str, season_line: Optional[str] = None):
    """Pretty HTML table with logos, retrying multiple logo URLs, % probs (one decimal),
       local time, team records. AWAY metrics first to align with top team."""
    if not preds:
        html = """
<!doctype html>
...
<body>
  <div class="wrapper">
    ...
    <div class="footer">Generated by <a href="https://x.com/reagankingisles">@ReaganKingIsles</a>. Logos © NHL/teams; loaded from NHL CDN.</div>
  </div>
<script>
